package main.java.com.molinari.utility.math;
...
public class UtilGeometry {
	 ...
	 *                        false otherwise.
	 */
	public static boolean isPointInsideRectangle (final int x0, final int y0, final int x1, final int y1,
	 ...
	 */
	public static boolean isPointInsidePolygon (final double[] x, final double[] y,
	 ...
	 * @return  True if the given point is inside the polygon, false otherwise.
	 */
	public static boolean isPointInsidePolygon (final int[] x, final int[] y,
	 ...
	 */
	public static double[] computePointOnLine (final double[] p0, final double[] p1, 
			final double fractionFromP0)
	{
	 ...
	 */
	public static double[] computePointOnLine (final double x0, final double y0,
			...
			final int x, final int y)
	{
		...
		final double u = ((x - x0) * (x1 - x0) + (y - y0) * (y1 - y0)) /
		(length * length);
	 ...
	 */
	private static int sameSide (final double x0, final double y0, final double x1, final double y1,
			final double px0, final double py0, final double px1, final double py1)
	{
		int  sameSide = 0;
	 ...
	 *                        >0 if points on same side.
	 */
	private static int sameSide (final int x0, final int y0, final int x1, final int y1,
			final int px0, final int py0, final int px1, final int py1)
	{
	 ...
	 */
	public static boolean isLineIntersectingLine (final int x0, final int y0, final int x1, final int y1,
	 ...
	 *                        false otherwise.
	 */
	public static boolean isLineIntersectingRectangle (final int lx0, final int ly0,
			final int lx1, final int ly1,
	 ...
	 *                        false otherwise.
	 */
	public static boolean isPolylineIntersectingRectangle (final int[] x, final int[] y,
			final int x0, final int y0,
			final int x1, final int y1)
	{
		...
		for (int i = 1; i < x.length; i++) {
			if (x[i-1] != x[i] || y[i-1] != y[i])
				if (isLineIntersectingRectangle (x[i-1], y[i-1],
						x[i], y[i],
						x0, y0, x1, y1))
					return true;
		}
	 ...
	 */
	public static boolean isPolygonIntersectingRectangle (final int[] x, final int[] y,
			final int x0, final int y0,
			final int x1, final int y1)
	{
		...
		if (isLineIntersectingRectangle (x[n-2], y[n-2], x[n-1], y[n-1],
				x0, y0, x1, y1))
			return true;
		...
		// or separate.
		//
		if (isPointInsidePolygon (x, y, x0, y0) ||
				isPointInsideRectangle (x0, y0, x1, y1, x[0], y[0]))
			return true;
	 ...
	 */
	public static void findPolygonExtent (final double[] x, final double[] y, final double[] z,
			final double[] xExtent,
	 ...
	 */
	public static void findPolygonExtent (final int[] x, final int[] y,
			final int[] xExtent,  // xMin, xMax
	 ...
	 */
	public static int findLineSegmentIntersection (final double x0, final double y0,
			final double x1, final double y1,
			...
			final double[] intersection)
	{
		...
		final double a0 = equals (x0, x1, LIMIT) ?
				INFINITY : (y0 - y1) / (x0 - x1);
		final double a1 = equals (x2, x3, LIMIT) ?
			...
			if (!equals (b0, b1))
				...
				if (equals (x0, x1)) {
					if (Math.min (y0, y1) < Math.max (y2, y3) ||
						...
						final double twoMiddle = y0 + y1 + y2 + y3 -
						min (y0, y1, y2, y3) -
						max (y0, y1, y2, y3);
						y = (twoMiddle) / 2.0;
						x = (y - b0) / a0;
					...
					if (Math.min (x0, x1) < Math.max (x2, x3) ||
						...
						final double twoMiddle = x0 + x1 + x2 + x3 -
						min (x0, x1, x2, x3) -
						max (x0, x1, x2, x3);
						x = (twoMiddle) / 2.0;
						y = a0 * x + b0;
	 ...
	 */
	public static double[] findLinePolygonIntersections (final double[] x, final double[] y,
			...
			final double x1, final double y1)
	{
		...
		for (int i = 0; i < nPoints; i++) {
			...
			final int type = findLineSegmentIntersection (x0, y0, x1, y1,
					x2, y2, x3, y3,
					intersection);
			...
			if (type == -2) { // Overlapping
				...
				final int side = sameSide (x0, y0, x1, y1,
						x[p1], y[p1], x[p2], y[p2]);
	...
	public static int[] createEllipse (final int[] x, final int[] y)
	{
		...
		final int[] ellipse = createEllipse (x0, y0, (int) Math.round (dx),
				(int) Math.round (dy));
		...
		// it to zero as well.
		if (!equals (axisAngle, Math.PI/2.0, 0.1) && 
				!equals (axisAngle, Math.PI,     0.1) &&
		...
		for (int i = 0; i < nPoints; i++) {
			final int xr = (int) Math.round (x0 +
					(ellipse[i*2+0] - x0) * cosAngle -
					(ellipse[i*2+1] - y0) * sinAngle);
			final int yr = (int) Math.round (y0 -
	 ...
	 * @return    Ellipse geometry [x,y,x,y,...].
	 */
	public static double[] createEllipse (final double x0, final double y0,
			double dx, double dy)
	{
		// Make sure deltas are positive
		dx = Math.abs (dx);
	 ...
	 */
	public static int[] createSector (final int x0, final int y0, final int dx, final int dy,
			final double angle0, final double angle1)
	{
	 ...
	 * @return        Geometry of secor [x,y,...]
	 */
	public static int[] createSector (final int x0, final int y0, final int radius,
			final double angle0, final double angle1)
	{
	 ...
	 */
	public static int[] createArrow (final int x0, final int y0, final int x1, final int y1,
			final double length, final double angle, final double inset)
	{
		final int[] x = {x0, x1};
		final int[] y = {y0, y1};    
	 ...
	 * @param height  Height (may be negative to indicaten upwards direction)
	 */
	public static double[] createRectangle (final double x0, final double y0,
	 ...
	 * @return             Geometry of star [x,y,x,y,...].
	 */
	public static double[] createStar (final double x0, final double y0,
