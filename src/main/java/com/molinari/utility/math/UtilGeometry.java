...
public class UtilGeometry {
	 ...
	 */
	public static boolean equals(final double a, final double b, final double limit) {
	 ...
	 */
	public static boolean isPointInsideRectangle(final int x0, final int y0, final int x1, final int y1, final int x,
			final int y) {
	 ...
	 */
	public static boolean isPointInsidePolygon(final double[] x, final double[] y, final double pointX,
			final double pointY) {
	 ...
	 */
	public static double[] computePointOnLine(final double x0, final double y0, final double x1, final double y1,
			final double fractionFrom0) {
		final double[] p0 = { x0, y0, 0.0 };
		final double[] p1 = { x1, y1, 0.0 };

		final double[] p = computePointOnLine(p0, p1, fractionFrom0);
	...
	public static void extendLine(final double[] p0, final double[] p1, final double toLength, final double anchor) {
		final double[] p = computePointOnLine(p0, p1, anchor);

		final double length0 = toLength * anchor;
		final double length1 = toLength * (1.0 - anchor);

		extendLine(p, p0, length0);
		extendLine(p, p1, length1);
	}
	...
	public static void extendLine(final double[] p0, final double[] p1, final double toLength) {
		final double oldLength = length(p0, p1);
	 ...
	 * 
	 * @param v
	 *            List to compute length of [x,y,z].
	...
	public static double length(final int x0, final int y0, final int x1, final int y1) {
		return length((double) x0, (double) y0, (double) x1, (double) y1);
	}
	...
	public static double distance(final int x0, final int y0, final int x1, final int y1, final int x, final int y) {
		...
		double length = length(x0, y0, x1, y1);
		if (length == 0.0)
			return length(x0, y0, x, y);
	...
	public static double computeAngle(final double[] p0, final double[] p1, final double[] p2) {
		...
		final double length1 = length(v0);
		final double length2 = length(v1);
	 ...
	 * 
	 * @param v0,
	 *            v1 Lists to compute dot product between [x,y,z].
	 ...
	 * 
	 * @param v0,
	 *            v1 Lists to compute cross product between [x,y,z].
	 * @param crossProduct
	 ...
	 */
	private static int sameSide(final double x0, final double y0, final double x1, final double y1, final double px0,
	 ...
	 *         exactly on the line >0 if points on same side.
	 */
	private static int sameSide(final int x0, final int y0, final int x1, final int y1, final int px0, final int py0,
			final int px1, final int py1) {
		return sameSide((double) x0, (double) y0, (double) x1, (double) y1, (double) px0, (double) py0, (double) px1,
				(double) py1);
	}
	 ...
	 */
	public static boolean isLineIntersectingLine(final int x0, final int y0, final int x1, final int y1, final int x2,
			final int y2, final int x3, final int y3) {
		final int s1 = sameSide(x0, y0, x1, y1, x2, y2, x3, y3);
		final int s2 = sameSide(x2, y2, x3, y3, x0, y0, x1, y1);
	 ...
	 */
	public static boolean isLineIntersectingRectangle(final int lx0, final int ly0, final int lx1, final int ly1,
			final int x0, final int y0, final int x1, final int y1) {
		// Is one of the line endpoints inside the rectangle
		if (isPointInsideRectangle(x0, y0, x1, y1, lx0, ly0) || isPointInsideRectangle(x0, y0, x1, y1, lx1, ly1))
			return true;
	 ...
	 */
	public static boolean isPolylineIntersectingRectangle(final int[] x, final int[] y, final int x0, final int y0,
		...
		for (int i = 1; i < x.length; i++) {
			if (x[i - 1] != x[i] || y[i - 1] != y[i])
				if (isLineIntersectingRectangle(x[i - 1], y[i - 1], x[i], y[i], x0, y0, x1, y1))
					return true;
		}
	 ...
	 * @return True if the polyline intersects the rectangle, false otherwise.
	 */
	public static boolean isPolygonIntersectingRectangle(final int[] x, final int[] y, final int x0, final int y0,
			final int x1, final int y1) {
		...
		if (n == 1)
			return isPointInsideRectangle(x0, y0, x1, y1, x[0], y[0]);

		//
		// If the polyline constituting the polygon intersects the rectangle
		// the polygon does too.
		//
		if (isPolylineIntersectingRectangle(x, y, x0, y0, x1, y1))
			return true;

		// Check last leg as well
		if (isLineIntersectingRectangle(x[n - 2], y[n - 2], x[n - 1], y[n - 1], x0, y0, x1, y1))
			return true;
		...
		// or separate.
		//
		if (isPointInsidePolygon(x, y, x0, y0) || isPointInsideRectangle(x0, y0, x1, y1, x[0], y[0]))
			return true;
	...
	public static double[] computePolygonCentroid(final double[] x, final double[] y) {
		...
		final double area = computePolygonArea(x, y);
	 ...
	 *            null) if z is null.
	 */
	public static void findPolygonExtent(final double[] x, final double[] y, final double[] z, final double[] xExtent,
	 ...
	 */
	public static int findLineSegmentIntersection(final double x0, final double y0, final double x1, final double y1,
			final double x2, final double y2, final double x3, final double y3, final double[] intersection) {
		...
		final double a0 = equals(x0, x1, LIMIT) ? INFINITY : (y0 - y1) / (x0 - x1);
		final double a1 = equals(x2, x3, LIMIT) ? INFINITY : (y2 - y3) / (x2 - x3);
			...
			if (!equals(b0, b1))
				...
				if (equals(x0, x1)) {
					if (Math.min(y0, y1) < Math.max(y2, y3) || Math.max(y0, y1) > Math.min(y2, y3)) {
						final double twoMiddle = y0 + y1 + y2 + y3 - min(y0, y1, y2, y3) - max(y0, y1, y2, y3);
						y = (twoMiddle) / 2.0;
						x = (y - b0) / a0;
					} else
						return -1; // Parallell non-overlapping
					...
					if (Math.min(x0, x1) < Math.max(x2, x3) || Math.max(x0, x1) > Math.min(x2, x3)) {
						final double twoMiddle = x0 + x1 + x2 + x3 - min(x0, x1, x2, x3) - max(x0, x1, x2, x3);
						x = (twoMiddle) / 2.0;
						y = a0 * x + b0;
					} else
						return -1;
	 ...
	 */
	public static double[] findLinePolygonIntersections(final double[] x, final double[] y, final double x0,
		...
		final double[] intersection = new double[2]; // Any given intersection
														// x,y
		...
		for (int i = 0; i < nPoints; i++) {
			...
			if (equals(x2, x3) && equals(y2, y3))
				continue;

			final int type = findLineSegmentIntersection(x0, y0, x1, y1, x2, y2, x3, y3, intersection);

			if (type == -2) { // Overlapping
				final int p1 = i == 0 ? nPoints - 1 : i - 1;
				final int p2 = next == nPoints - 1 ? 0 : next + 1;

				final int side = sameSide(x0, y0, x1, y1, x[p1], y[p1], x[p2], y[p2]);
	...
	public static int[] createEllipse(final int[] x, final int[] y) {
		...
		final double axisAngle = computeAngle(p0, p1, p2);

		// dx / dy
		final double dx = length(x0, y0, x[1], y[1]);
		final double dy = length(x0, y0, x[0], y[0]) * Math.sin(axisAngle);
		...
		// it to zero as well.
		if (!equals(axisAngle, Math.PI / 2.0, 0.1) && !equals(axisAngle, Math.PI, 0.1)
				&& !equals(axisAngle, 0.0, 0.1)) {
	...
	public static double[] createEllipse(final double x0, final double y0, double dx, double dy) {
		...
		// Find the angle step
		final double angleStep = Math.PI / 2.0 / nPoints;
	 ...
	 */
	public static int[] createSector(final int x0, final int y0, final int dx, final int dy, final double angle0,
			final double angle1) {
	 ...
	 * @return Geometry of secor [x,y,...]
	 */
	public static int[] createSector(final int x0, final int y0, final int radius, final double angle0,
			final double angle1) {
	 ...
	 */
	public static int[] createArrow(final int[] x, final int[] y, final double length, final double angle,
			final double inset) {
		...
		findPolygonPosition(x, y, length, pos1);
		...
		v = dx > 0.0 && dy <= 0.0 ? Math.PI + v
				: dx > 0.0 && dy >= 0.0 ? Math.PI - v : dx <= 0.0 && dy < 0.0 ? -v : dx <= 0.0 && dy > 0.0 ? +v : 0.0;
	 ...
	 */
	public static int[] createArrow(final int x0, final int y0, final int x1, final int y1, final double length,
			final double angle, final double inset) {
		final int[] x = { x0, x1 };
		final int[] y = { y0, y1 };
	...
	public static int[] createRectangle(final int x0, final int y0, final int width, final int height) {
		return new int[] { x0, y0, x0 + (width - 1), y0, x0 + (width - 1), y0 + (height - 1), x0, y0 + (height - 1), x0,
				y0 };
	}
	 ...
	 */
	public static int[] createStar(final int x0, final int y0, final int innerRadius, final int outerRadius,
			final int nArms) {
	 ...
	 * @return Geometry of star [x,y,x,y,...].
	 */
	public static double[] createStar(final double x0, final double y0, final double innerRadius,
			final double outerRadius, final int nArms) {
		final int nPoints = nArms * 2 + 1;
	...
	public static boolean findPolygonPosition(final int[] x, final int[] y, final double length, final int[] position) {
		...
		for (int i = 1; i < x.length; i++) {
			final double legLength = length(x[i - 1], y[i - 1], x[i], y[i]);
